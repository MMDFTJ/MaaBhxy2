...
def count_draw_coordinate(rectPts):
    ...
    return (
    (int(rectPts[0][0] * z - x), int(rectPts[0][1] * z - y)), (int(rectPts[1][0] * z - x), int(rectPts[1][1] * z - y)))
...
while True:
    ...
    for roi in crop_list:
        ...
        if needSave:
            ...
            dst_filename: str = f'活动截图.png'
            print(f"dst: ..\resource\image\{dst_filename}")
            cv2.imwrite('..\\resource\\image\\' + dst_filename, roi.image)
