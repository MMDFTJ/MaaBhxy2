...
class ErrorDecorator:
    ...
    def __call__(self, func):
        def wrapper(self_of_instance, *args, **kwargs):
            bool_signal = getattr(self_of_instance, self.attr_name)
            if bool_signal == 2:
                raise TypeError
            return func(self_of_instance, *args, **kwargs)
...
class DecoratorMeta(type):
    def __new__(cls, name, bases, dct):
        for key, value in dct.items():
            if hasattr(value, 'decorator_attr'):
                dct[key] = ErrorDecorator(value.decorator_attr)(value)
        return super().__new__(cls, name, bases, dct)


def dynamic_error_decorator(attr_name):
    def decorator(func):
        func.decorator_attr = attr_name
        return func

    return decorator
###

class Tasker(metaclass=DecoratorMeta):
